...
class MultiHeadAttention(nn.Module):
    ...
    def split_multiple_heads(self, tensor: torch.Tensor):
        ...
        tensor = tensor.reshape(batch_size, seq_len, self.n_heads, emb_dim // self.n_heads)
        # (B, N, n_head, splitted_dim) -> (B, n_head, N, splitted_dim)
        tensor = tensor.transpose(1, 2)
        return tensor
    
    def scaled_dot_proudction(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor):
